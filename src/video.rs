...
impl RetroVideo {
    ...
    ) -> Result<RetroVideo, Box<dyn Error>> {
    ...
    pub fn draw_new_frame(&mut self) {
        unsafe {
            if let Some((gl_context, gl_surface, win)) = &self.state {
    ...
    pub fn resume(&mut self, window_target: &EventLoopWindowTarget<()>) {
        ...
        //     self.av_info.video.geometry.max_width.,
        //     self.av_info.video.geometry.max_height,
        // )));
